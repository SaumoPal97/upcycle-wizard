/*
  ...
  1. New Data
    ...
    - Creates corresponding steps for each project
    - Ensures projects have proper guide_json structure

  2. Sample Projects
    - Vintage Dresser Makeover (Bohemian style)
    - Coffee Table Restoration (Industrial style)  
    - Bookshelf Makeover (Modern style)
...
-- Create sample user if not exists (for demo purposes)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM auth.users WHERE email = 'demo@upcyclewizard.com'
  ) THEN
